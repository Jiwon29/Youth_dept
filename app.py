...
"""
    ...
    with info2:
        st.markdown(
            '<div class="insight-box">💡 <b>저금리가 불러온 투자 열풍 — 데이터로 확인</b><br><br>'
            '청년층의 주식·자산투자 목적 부채 총량은 '
            '<b>2018년 약 753억원에서 2021년 약 4,166억원으로 약 5.5배 급증</b>했습니다. '
            '기준금리가 0.5%까지 내려간 초저금리 시기, 청년들이 본격적으로 '
            '레버리지 투자에 뛰어들었음이 실제 데이터로 확인됩니다.<br><br>'
            '그렇다면 이 부채는 모두에게 같은 결과를 가져다줬을까요? '
            '소득 수준에 따라 결과가 달랐을 수 있습니다.</div>',
            unsafe_allow_html=True)
        with st.expander("🗄️ SQL 쿼리 보기"):
    ...
    st.markdown(
        ...
        '초저금리(2020~2021)는 청년층의 투자 목적 부채를 단기간에 약 5.5배 폭증시켰습니다. '
        '같은 시기 청년들은 동일한 저금리 환경에서 같은 방식으로 빚을 냈지만, '
        '<b>그 결과가 모두에게 동일했는지는 별개의 문제입니다. '
        '소득 수준에 따라 부채가 자산이 됐는지, 함정이 됐는지 → 대시보드 2에서 확인하세요.</b></div>',
        unsafe_allow_html=True)
